...
// Interface for history items from the history page
...
// Single item export (for generate page)
export const generateCSV = (
    productName: string, 
    variants: string[], 
    format: CSVFormat = 'general',
    ...
    return generateCSVFromData(data, format);
};

// Batch export from history page
export const generateCSVFromHistory = (
    items: HistoryItem[], 
    format: CSVFormat = 'general'
): string => {
    const data: CSVData[] = items.map(item => ({
...
// Internal function to generate CSV from data array
const generateCSVFromData = (data: CSVData[], format: CSVFormat): string => {
    let headers: string[] = [];
    let rows: string[][] = [];
