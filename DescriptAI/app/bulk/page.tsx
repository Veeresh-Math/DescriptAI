...
export default function BulkPage() {
  ...
  const loadSampleProducts = () => {
    const sample = `name,features,category
...
Laptop Stand,Ergonomic design, adjustable height, laptop cooling,Accessories
Coffee Maker,Automatic brewing, timer, keep warm function,Kitchen
Yoga Mat,Non-slip, eco-friendly material, carrying strap,Fitness`;
    setCsvData(sample);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
          ...
          <div className="flex items-center justify-between h-14 sm:h-16">
            <div className="flex items-center gap-4 sm:gap-8">
              ...
              <div className="hidden md:flex gap-6">
                <Link href="/generate" className="text-white/70 hover:text-white transition text-sm sm:text-base">Generate</Link>
                <Link href="/history" className="text-white/70 hover:text-white transition text-sm sm:text-base">History</Link>
                <Link href="/pricing" className="text-white/70 hover:text-white transition text-sm sm:text-base">Pricing</Link>
                <Link href="/bulk" className="text-cyan-400 font-medium text-sm sm:text-base">Bulk</Link>
              </div>
            </div>
            <SignedIn>
            ...
            <SignedOut>
              <Link href="/sign-in" className="text-white/70 hover:text-white text-sm sm:text-base">Sign In</Link>
            </SignedOut>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-3 py-8 sm:px-4 sm:py-12">
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="bg-white/10 rounded-lg p-1 flex">
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-md transition text-sm sm:text-base ${
        ...
        {/* CSV Input */}
        {activeTab === 'csv' && (
          <div className="bg-white/10 rounded-2xl p-4 sm:p-8 mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl font-semibold text-white mb-3 sm:mb-4">Upload CSV File</h2>
            ...
            <div className="mb-4 sm:mb-6 flex flex-wrap gap-3 sm:gap-4">
              ...
              <label
                ...
                className="inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg cursor-pointer transition text-sm sm:text-base"
              >
                📁 Upload CSV File
              </label>
              <button
                onClick={loadSampleProducts}
                className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg transition"
              >
                🎯 Try Sample
              </button>
              <span className="ml-3 text-white/60 text-sm self-center">
                {products.length > 0 ? `✅ ${products.length} products loaded!` : 'Upload CSV or try sample'}
              </span>
            </div>

            <p className="text-white/40 text-sm mb-2">Or paste CSV data manually:</p>
            <textarea
              value={csvData}
              onChange={(e) => setCsvData(e.target.value)}
              placeholder={`name,features,category\nWireless Headphones,Bluetooth 5.0, noise cancelling, 30hr battery,Electronics\nSmart Watch,Fitness tracker, heart rate monitor, waterproof,Wearables`}
              className="w-full h-32 bg-black/30 border border-white/20 rounded-lg p-4 text-white placeholder-white/40 focus:outline-none focus:border-cyan-500 font-mono text-sm"
            />
          </div>
        )}
