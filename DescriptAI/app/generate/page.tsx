...
export default function GeneratePage() {
    ...
    useEffect(() => {
        ...
        // Check if user came from pricing page with tier selection
        const tierParam = searchParams.get("tier");
        if (tierParam && ["free", "pro", "agency"].includes(tierParam.toLowerCase())) {
            // Update user's tier in the database
            fetch("/api/user", {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ tier: tierParam.toLowerCase() })
            }).then(() => {
            ...
            <header className="bg-white border-b border-pink-100 py-3 px-4 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex gap-3 items-center">
                        ...
                        <Link href="/bulk" className="text-xs font-bold text-amber-700 hover:text-amber-900 transition p-2">🚀 Bulk</Link>
                        <Link href="/team" className="text-xs font-bold text-teal-700 hover:text-teal-900 transition p-2">👥 Team</Link>
                        <Link href="/analytics" className="text-xs font-bold text-purple-700 hover:text-purple-900 transition p-2">📊 Analytics</Link>
                        {userData && (
