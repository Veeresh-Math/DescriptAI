...
export async function POST(req: Request) {
    try {
        ...
        // Parse body first for demo check
        let body: any;
        try {
            body = await req.json();
        } catch {
        ...
        // Custom Keywords and Brand Voice are Pro/Agency features (not Free)
        ...
        // Social Media Kit is available for Pro and Agency tiers only
...
REMEMBER: You're not writing a description - you're writing a SALES MACHINE that happens to look like a description. Make every word pull its weight!`;
        ...
        // Try DeepSeek first (PRIMARY - FREE) - with timeout
        ...
        try {
            ...
            // Fallback to Groq (faster than DeepSeek)
            try {
                ...
                try {
                    ...
                    // Use static template as final fallback (instant)
        ...
        // Skip slow web crawler for faster results - use AI only
        // (Web crawler can take 5-15 seconds, skipping for speed)
