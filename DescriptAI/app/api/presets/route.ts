...
export async function GET() {
    ...
    try {
        ...
        isAgency = false; // Default to false when database is unavailable
    }

    // Agency-only feature enforcement
    if (!isAgency) {
        return NextResponse.json(
            { error: "UPGRADE_REQUIRED", message: "Brand Voice Presets are an Agency-tier feature. Upgrade to unlock!" },
            { status: 403 }
        );
    }
