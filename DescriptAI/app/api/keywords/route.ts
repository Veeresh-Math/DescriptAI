...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Check if user is Pro or Agency (Protected by try-catch with fallback)
        let isPremium = false;
        try {
            const user = await db.user.findUnique({ where: { id: userId } });
            isPremium = user?.tier === 'pro' || user?.tier === 'agency';
        } catch (dbError) {
            console.error("[KEYWORDS_DB_FALLBACK]", dbError);
            isPremium = false; // Default to false when database is unavailable
        }

        // Pro+ feature enforcement
        if (!isPremium) {
            return NextResponse.json(
                { error: "UPGRADE_REQUIRED", message: "SEO Keyword Suggestions require Pro or Agency tier. Upgrade to unlock!" },
                { status: 403 }
            );
        }
